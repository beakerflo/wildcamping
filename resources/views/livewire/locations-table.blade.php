    ...
    <div class="w-2/3 mr-6">
        ...
        <div class="border PrimaryBorderColor rounded-md rounded-t-none divide-y divide-gray-300">
            @foreach($Locations as $Location)
                <div wire:click.prevent="getLocationDetails({{ $Location->id }})" class="TableRow hover:bg-gray-200 space-x-1">
        ...
        <div class="w-1/3 border PrimaryBorderColor rounded-md shadow-md max-h-screen p-2">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-start">
                    <div class="TextHeader px-1 space-x-1">{{ $LocationDetails->name }}</div>
                    <div><span class="flex text-xs">door: </span>@foreach($LocationDetails->sources as $source) <div>{{ $source->name }} @endforeach</div></div>
                </div>
                <div>
                    <ul class="flex flex-wrap space-x-1">
                        @if($LocationDetails->tags->count() > 0)
                            @foreach($LocationDetails->tags as $tag)
                ...
                <div>
                    ...
                    <div class="flex">
                        ...
                        <div class="w-1/2">
                            @if ($LocationDetails->images->count() > 0)
                                <img src="/img-loc/{{ $LocationDetails->images[0]->filename }}" alt="{{ $LocationDetails->images[0]->name }}" />
                            @endif
                        </div>
                    </div>
                </div>
                <div class="flex">
                    <div class="w-1/2">
                        ...
                        <div class="flex space-x-4">
                            ...
                            <div>{{ $LocationDetails->coordinate->latitude }}</div>
                        </div>
                        <div class="flex space-x-4">
                            <div class="SubText px-1">lng:</div>
                            <div>{{ $LocationDetails->coordinate->longitude }}</div>
                        </div>
                        <div class="flex space-x-4">
                            <div class="SubText px-1">{{ __('Map') }}:</div>
                            <div class="flex space-x-2">
                                <a href="https://www.google.nl/maps/place/{{ $LocationDetails->coordinate->latitude }},{{ $LocationDetails->coordinate->longitude }}" target="_blank">
                                ...
                                <a href="https://duckduckgo.com/?q={{ $LocationDetails->coordinate->latitude }}%2C+{{ $LocationDetails->coordinate->longitude }}&iaxm=maps&ia=web" target="_blank">
                    ...
                    <div class="w-1/2">
                        ...
                        <div class="flex space-x-2 items-center">
                            ...
                            <div>{{ $LocationDetails->coordinate->address->postcode }}</div>
                        </div>
                        <div class="flex space-x-2 items-center">
                            <div class="SubText">{{ __('Region') }}:</div>
                            <div>{{ $LocationDetails->coordinate->address->part }}</div>
                        </div>
                        <div class="flex space-x-2 items-center">
                            <div class="SubText">{{ __('City') }}:</div>
                            <div>{{ $LocationDetails->coordinate->address->city }}</div>
                        </div>
                        <div class="flex space-x-2 items-center">
                            <div class="SubText">{{ __('Area') }}:</div>
                            <div>{{ $LocationDetails->coordinate->address->state }}</div>
                        </div>
                        <div class="flex space-x-2 items-center">
                            <div class="SubText">{{ __('Country') }}:</div>
                            <div class="flex space-x-3">
                                <div>{{ $LocationDetails->coordinate->address->country->name }}</div>
                                {!! $LocationDetails->getFlagSvg(4) !!}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="divide-y divide-grey-300">
                    ...
                    @if($LocationDetails->visits->count() > 0)
                        @foreach($LocationDetails->visits as $visit)
